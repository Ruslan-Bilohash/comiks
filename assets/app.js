...
(() => {
  ...
  function renderHome() {
    ...
    const modules = h('section', {}, h('h2', { class: 'sec-title' }, t('modules_title')),
      h('div', { class: 'module-grid' }, ['run', 'hunt', 'race', 'chess', 'game', 'words', 'grammar', 'english', 'math', 'alphabet', 'numbers', 'plan', 'tests', 'rating'].filter(k => mods[k]).map(k => h('a', { class: 'module m-' + k, href: k === 'hunt' ? '#/hunt' : (k === 'run' ? '#/run' : '#/' + k) }, window.KomiksIcons && window.KomiksIcons.MODULES[k] ? window.KomiksIcons.badge(...window.KomiksIcons.MODULES[k], 30) : h('b', {}, mods[k][0]), h('span', {}, mods[k][1]), h('small', {}, mods[k][2])))));
  ...
  function renderFooter() {
    ...
    foot.replaceChildren(
      h('div', { class: 'foot-inner' },
        ...
        col(FX.games, [link('#/run', '🏃 ' + (navT('run') || 'Skattejakt')), link('#/hunt', '💬 ' + (navT('hunt') || 'Boblejakt')), link('#/race', '🏁 ' + navT('race')), link('#/chess', '♟ ' + navT('chess')), link('#/math/rocket', '🚀 Math Rocket'), link('#/math/race', window.KomiksRocket ? window.KomiksRocket.text('with_class') : '👥 Math Rocket'), link('#/game', navT('game')), link('#/tests', navT('tests')), link('#/rating', navT('rating'))]),
